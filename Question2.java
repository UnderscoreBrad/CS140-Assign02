...
public class Question2 {
    public static void main(String[] args)
    {
        ...
        for (int[] row : arr2) System.out.println(Arrays.toString(row));
        var arr3 = new Question2(12).makePascalTriangle();
        for (int i = 0; i < 12; i++)
        {
            for(int j = 0; j < 11 - i; j++) System.out.print("  ");
            for(int j = 0; j < arr3[i].length; j++) System.out.printf("%4d", arr3[i][j]);
            System.out.println();
        }
        System.out.println("-----------------------------------");
        int[][] matrix = {
        ...
        for (int i = 0; i < matrix.length; i++)
            System.out.println("Row " + i + " equal: "
                    + Arrays.equals(matrix[i], arr4[i]));
    }

    private int length1;
    private Random rand = new Random();

    public Question2 (int length1){
    ...
    public String[] makeStringArray()
    {
        ...
        {
            byte[] bArray = new byte[length1 + 1];
            for (int j = 0; j < i+1; j++)
            {
    ...
    public static int[] makeRowMajor(int[][] array){
        ...
        for (int i = 0; i < array.length; i++){
            ...
            }else if(array[i].length!=numCols){
                throw new IllegalArgumentException("Array is ragged");
            }
        }
        var retVal = new int[2+numCols*array.length];
    ...
    public static int[][] unfoldRowMajor(int[] array) {
        ...
        var retArray = new int[numRows][numCols];
